...
export default component$<AutocompleteInputProps>(
  ({
    id,
    label,
    placeholder,
    ...
    const autocomplete =
      useSignal<NoSerialize<google.maps.places.Autocomplete> | null>(null);
    ...
    const handlePlaceSelected = $(async () => {
      ...
      const address =
        place.formatted_address || place.name || inputRef.value?.value || "";
      if (value) {
        value.value = address;
      }
      if (onPlaceSelected$) {
      ...
      const tryInit = () => {
        ...
        if (!initialized.value && val.length >= minChars && inputRef.value) {
          if (!(globalThis as any).google?.maps?.places) {
            console.warn(
              "[AutocompleteInput] google.maps.places is not available yet",
            );
            return;
          }
          // Initialize autocomplete only after minChars
          autocomplete.value = noSerialize(
            new google.maps.places.Autocomplete(inputRef.value, {
              fields: [
                "formatted_address",
                "geometry",
                "name",
          ...
          console.debug(
            "[AutocompleteInput] Autocomplete initialized (minChars met)",
          );
          const ac = autocomplete.value as unknown as {
    ...
    return (
      <div>
        ...
        <input
          ...
          value={value?.value}
          onInput$={(e) =>
            value && (value.value = (e.target as HTMLInputElement).value)
          }
        ...
        <p class="mt-1 text-xs text-gray-500">
          Escribe al menos {minChars} caracteres para ver sugerencias.
        </p>
        {error.value && <p class="mt-1 text-sm text-red-600">{error.value}</p>}
      </div>
    );
  },
);
